...
Unit.prototype.update = function() {
   ...
   Entity.prototype.update.call(this);
   if (this.side === SMALL || this.side === BIG) {
       this.counter += this.game.clockTick * 1;
       this.health -= this.game.clockTick * 1; 
   }
   ...
   if (this.side === FOOD) {
    ...
    if (this.x < 0) {
        removed = true;
    } else if (this.x + this.currentAction.frameWidth > canvasWidth) {
        removed = true;
    }
    if (this.y < 0) {
        removed = true;
    } else if (this.y + this.currentAction.frameHeight > canvasHeight) {
        removed = true;
    }
   ...
   if (this.side === SMALL) {
       if (this.counter > 10 && this.health > 10) {
           spawnUnit(this.game, this.x, this.y, this.unitcode, SMALL);
           this.counter = 0;
       } 

    if (this.x < 0) {
        this.velocity.x = this.movementspeed;
        this.changeAction("move_right");
    } else if (this.x + this.currentAction.frameWidth > canvasWidth) {
        this.velocity.x = -this.movementspeed;
        this.changeAction("move_left");
    }
    if (this.y < 0) {
